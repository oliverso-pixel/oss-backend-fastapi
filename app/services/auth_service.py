...
class AuthService:
    ...
    def refresh_access_token(self, refresh_token: str) -> Token:
        ...
        try:
            ...
            # 檢查 refresh token 是否在黑名單中（使用 Redis）
            if refresh_jti and self.blacklist_service.is_blacklisted(refresh_jti):
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
                    detail="Refresh token has been revoked"
                )
        ...
        # 使用資料庫事務來確保原子性
        try:
            # 將所有舊的 access token 的 JTI 加入黑名單（使用 Redis）
            old_access_tokens = self.db.query(UserToken).filter(
                UserToken.user_id == user.id,
                UserToken.token_type == TokenType.ACCESS,
                UserToken.expires_at > datetime.utcnow(),
    ...
    def revoke_all_user_tokens(self, user_id: int, reason: str = "Password changed"):
        ...
        # 將用戶的所有 tokens 加入 Redis 黑名單
        self.blacklist_service.blacklist_user_tokens(
            user_id=user_id,
            reason=reason
        )
