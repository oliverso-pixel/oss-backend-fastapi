...
from app.schemas.pet import (
    ...
    PetListResponse, PetStatistics,
    PetPublicResponse, PetPrivateResponse
)
...
def search_pets(
    ...
    """搜索寵物（public和private皆可搜，但顯示不同資訊）"""
    ...
    # 排除封鎖了當前用戶的飼主所擁有的寵物
    blocker_ids_subquery = social_service.get_user_blocker_ids_subquery(current_user.id)
    query = query.filter(Pet.user_id.notin_(blocker_ids_subquery))
    ...
    visible_pets = []
    for pet in pets:
        visible_data = privacy_service.get_pet_data_for_viewer(current_user, pet)
        if visible_data:
            visible_pets.append(visible_data)
...
def get_pet(
    ...
    # 使用 privacy_service 決定回傳哪種格式的資料
    response_data = privacy_service.get_pet_data_for_viewer(current_user, pet)
